<?php
	Class Buscarcontroldinerorecolector{
		private $Comunicacion;
		function Buscarcontroldinerorecolector(){
			include("../../php/conexion.php");
			$this->Comunicacion = new Conexion();
			if(isset($_POST["fechainicial"]) && isset($_POST["fechafinal"]) && isset($_GET["usuario"])){
		...
		private function retorno($fecha1,$fecha2){
			...
			<tr><th>Taquillas</th><th>Ganancia</th><th>Premios A Responder</th><th>Dinero A Entregar</th><th>Total A Cobrar O Pagar</th></tr>";
			...
			$consultabancas = $this->Comunicacion->Consultar("SELECT DISTINCT banca FROM controldinero WHERE usuariocreador='".$this->NombreDeRecolector($_GET["usuario"])."' AND pagorecolector<>'si' ORDER BY banca LIMIT 0,".$cantidadregistromostrar."");
			...
			$r=$r."</div><br>";
			$consultabotones = $this->Comunicacion->Consultar("SELECT DISTINCT banca FROM controldinero WHERE usuariocreador='".$this->NombreDeRecolector($_GET["usuario"])."' AND pagorecolector<>'si' ORDER BY banca LIMIT 0,".$cantidadregistromostrar."");
	...
	new Buscarcontroldinerorecolector();
?>