<?php
	Class Desbloquearrecolector{
		private $Comunicacion;
		function Desbloquearrecolector(){
			include("../../php/conexion.php");
			$this->Comunicacion = new Conexion();
			if(isset($_GET["usuariodesbloquear"])){
				if($this->Desbloquear()=="bien"){
					echo $this->ListadoRecolector();
				}
			}
		}
		private function Desbloquear(){
			$consulta = $this->Comunicacion->Consultar("UPDATE recolectores SET bloqueo='no' WHERE usuario='".$_GET["usuariodesbloquear"]."'");
			$consultar = $this->Comunicacion->Consultar("UPDATE usuarios SET bloqueo='no' WHERE nick='".$_GET["usuariodesbloquear"]."'");
			$consularbancas = $this->Comunicacion->Consultar("SELECT usuario FROM bancas WHERE usuariocreador='".$_GET["usuariodesbloquear"]."'");
			while($bancabloquear = $this->Comunicacion->Recorrido($consularbancas)){
				$this->Comunicacion->Consultar("UPDATE bancas SET bloqueo='no' WHERE usuario='".$bancabloquear[0]."'");
				$this->Comunicacion->Consultar("UPDATE usuarios SET bloqueo='no' WHERE nick='".$bancabloquear[0]."'");
			}
